...
if($_POST["search"]){
            ...
            $sql = " SELECT IDD,Name ,NickName,Address,Country,Telephone from declarant 			
			WHERE  Name='".$_POST['search']."'";
			 $sqlc = "select count(*) from declarant where Name='".$_POST['search']."'"; // 計算資料表 nwsuppliers 總筆數
...
<body>
<h4><strong><a href="#" >資料首頁</a> > <a href="#" >系統檔案維護</a> > <a href="DeclarantVeiw.php" >客戶資料</a> > <a href="#" >檢視資料</a></strong></h4>
<hr />
<form action="DeclarantVeiw.php" method="post" >
<input type="text" name="search" id="search" value="ex: 請輸入姓名搜尋"  />
<input type="submit"  value="搜尋"  />


</form>

<input type="button"  value="新增"  onclick="location.href='DeclarantEdit.php'"/>
    <form action="DeclarantVeiw.php" method="post" name="sd">
...
<h4>
    <table class="table table-striped table-bordered table-condensed" align='center' >
        <tr align='center' > 
        <td nowrap class="col-sm-2">客戶名稱</td>
        <td nowrap class="col-sm-1"></td>
        <td nowrap class="col-sm-1"></td>
        <td nowrap class="col-sm-2">暱稱</td>
        <td nowrap class="col-sm-2">地址</td>
		<td nowrap class="col-sm-2">國家</td>
        ...
        ?>
        <tr align='center'>
            <td nowrap ><a href="DeclarantShow.php?IDD=<?php echo $CusIDD ;?>"><?php echo $CusName;?></a></td>
            <td nowrap ><input type="button" name="Edit" id="Edit" value="編輯" onClick="if(confirm('您確定編輯?'))window.location.href='DeclarantEdit.php?IDD=<?php echo $CusIDD; ?>';else return false"/> </td>
        	<td nowrap ><input type="button" name="Delete" id="Delete" value="刪除" onClick="if(confirm('確定要刪除?'))window.location.href='DeclarantDelete.php?IDD=<?php echo $CusIDD; ?>';else return false" /></td>
